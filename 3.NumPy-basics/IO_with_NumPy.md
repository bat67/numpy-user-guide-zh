...
- [NumPy I/O操作](#numpy-io操作)
  - [使用genfromtxt导入数据](#使用genfromtxt导入数据)
    - [定义输入](#定义输入)
    - [将行拆分为列](#将行拆分为列)
      - [``delimiter``参数](#delimiter参数)
      - [``autostrip``参数](#autostrip参数)
      - [``comments``参数](#comments参数)
    - [跳过直线并选择列](#跳过直线并选择列)
      - [``skip_header``和``skip_footer``参数](#skip_header和skip_footer参数)
      - [``usecols``参数](#usecols参数)
    - [选择数据的类型](#选择数据的类型)
    - [设置名称](#设置名称)
      - [``names``参数](#names参数)
      - [``defaultfmt``参数](#defaultfmt参数)
      - [验证名称](#验证名称)
    - [调整转换](#调整转换)
      - [``converters``参数](#converters参数)
      - [使用缺失值和填充值](#使用缺失值和填充值)
